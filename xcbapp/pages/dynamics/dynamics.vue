		...
		<scroll-view class="timeline-container" scroll-y @scrolltolower="loadMore">
			...
			<view v-for="(group, date) in timelineGroups" :key="date" class="timeline-group">
				...
				<view class="timeline-cards">
					<view v-for="(post, index) in group" :key="index" class="neo-card">
						<!-- 原有卡片内容 -->
						...
						<scroll-view class="media-scroller" scroll-x :scroll-with-animation="true"
							:show-scrollbar="false">
							<view class="media-container">
								<view v-for="(item, idx) in post.imgs" :key="idx" class="media-item">
									<image v-if="item.fileType === 'image'" :src="item.path" mode="aspectFill"
										class="media-content" @click="viewImage(item.path)" />
									<video v-else :src="item.path" controls class="media-content"
							...
							<view class="interaction-panel">
								<view class="action-btn" @click="toggleReaction(post.id)">
									<uni-icons :type="post.liked ? 'heart-filled' : 'heart'"
										:color="post.liked ? '#FF6B6B' : '#7A9D7E'" size="20" />
									<text class="count">{{post.likes}}</text>
								</view>
								<view class="action-btn">
									<uni-icons type="chat" color="#7A9D7E" size="20" />
