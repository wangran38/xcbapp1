...
const _sfc_main = {
  ...
  methods: {
    async fetchData(params) {
      const response = await api_index.api.myShoplist({
        ...params,
        isshow: 1
      });
    ...
    async off(itemId) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/Listed-Dishes/Listed-Dishes.vue:104", "下架失败", error);
      }
    }
  }
};
  ...
  return common_vendor.e({
    a: common_vendor.f($data.pageData, (item, k0, i0) => {
      return common_vendor.e($data.isloaded ? {
        ...
        d: common_vendor.t(item.weight_name),
        e: common_vendor.t(item.isshow === 1 ? "下架" : "已下架"),
        f: item.isshow === 0 ? 1 : "",
        g: common_vendor.o(($event) => item.isshow === 1 && $options.off(item.id), item.id),
        h: item.id
      });
    }),
    b: $data.isloaded,
